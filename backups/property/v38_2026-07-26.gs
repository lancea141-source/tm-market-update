 ...
 * ------------------------------------------------
 * CHANGELOG
 *  v38 BASEMENT-RATE CAP + MONTH/YEAR HEADER + MONTHLY SNAPSHOT HISTORY.
 *      · BASEMENT CAP. cmaModel_() and cmaTHModel_() could occasionally let
 *        the finished-basement $/sqft rate exceed the above-grade rate in
 *        markets with thin basement variation — a buyer paying more for a
 *        basement than for the same square footage upstairs is not a real
 *        market signal, it's a symptom of a free-fit hedonic model with too
 *        little data to constrain that one coefficient. fin is now capped
 *        at min(fit value, l1) — a cap, not a re-derivation, so it only
 *        intervenes when the model actually misbehaves; markets where fin
 *        already sits sensibly below l1 (TM, Lehi) are untouched. When the
 *        cap fires, cmaRows_()/cmaRowsTH_() print a visible note on both the
 *        internal CMA tab and the public cma-rates/ page, and the raw fitted
 *        (uncapped) value is kept in the model object for anyone who wants
 *        to see it — the number just isn't published or used in CMA math.
 *      · MONTH/YEAR HEADER. The index card now shows the report month/year
 *        as prominently as the index number itself (stacked above it, same
 *        size), so a report — especially an archived one — announces which
 *        month it covers at a glance instead of requiring the visitor to
 *        read the small "Updated" line.
 *      · MONTHLY SNAPSHOT HISTORY. Every successful publish now also saves
 *        a copy to <location>/history/<yyyy-MM>/ — overwritten on repeat
 *        publishes within the same month, frozen once the month rolls over.
 *        A small <location>/history/index.html lists every saved month,
 *        newest first, and the live report footer links to it via
 *        "📅 Past Reports". Wrapped in try/catch, same as the registry call
 *        — a snapshot hiccup never blocks the real publish.
 *      · REVERTED (from v37): self-backup-via-Apps-Script-API. Confirmed
 *        blocked by script.googleapis.com's 403 under the default
 *        auto-created Cloud project every bound script gets — fixing it
 *        needs migration to a standard user-managed GCP project, which is
 *        out of proportion to what the feature bought. backupSourceToGithub_()
 *        is left in the file, unused, as a starting point if that migration
 *        is ever worth doing. Script Link in the registry now always points
 *        straight at the script editor.
 *  v37 LINK-PREVIEW IMAGES + PROJECT REGISTRY TRACKER.
 *      · og:image / og:title / twitter:card meta tags added to buildHtml_(),
 *        buildCmaHtml_() and buildCompareHtml_() — texted/shared links now
 *        show the logo instead of no preview at all. Points at the same
 *        public logo asset the land engine publishes (assets/logo.svg),
 *        not the Drive-hosted logoFileId (that uc?export=view format is
 *        not reliably readable by link-preview crawlers).
 *      · registerToTracker_() appended — every 🌐 Publish now writes/updates
 *        one row in the shared Project Registry (Dashboard + History +
 *        Archive), keyed by Project/Pipeline/Area. Wrapped in try/catch so
 *        a registry hiccup never blocks the actual report publish.
 *      · SELF-BACKUP VIA APPS SCRIPT API. On publish, if this VERSION hasn't
 *        been backed up yet (tracked via Script Property BACKED_UP_VERSION),
 *        the script reads its OWN source through script.googleapis.com and
 *        pushes it to backups/property/{slug}/{farmShort}_{VERSION}_{date}.gs
 *        on GitHub — fully automatic, no manual copy/paste. The resulting
 *        raw GitHub URL is stored and used as the registry's Script Link,
 *        so the registry always has a live link to the exact source that
 *        produced that report. Backs up once per version, not every publish.
 *      · Requires two one-time account settings (see backupSourceToGithub_()
 *        below): enable the Apps Script API at
 *        script.google.com/home/usersettings, and accept the OAuth consent
 *        prompt on each sheet's first publish after this update.
 *  v36 NEW-MARKET RESET + IMPORT AUDIT + VERSION ON PUBLISHED PAGES.
 *      · 🆕 Set Up New Market (menu + Run tab): the safe way to repurpose a
 *        DUPLICATED sheet for a different city. Type the new market name to
 *        confirm, then it clears Data, Import Log, cached stats, the
 *        inherited LEAD_FORM_URL (otherwise the new market's CTA button
 *        still points at the OLD city's form and leads land in the wrong
 *        spreadsheet), and the stored LOCATION — then sets the new location
 *        in the same step. KEEPS the GitHub token. Form Responses, if the
 *        duplicate carried one, gets its own separate confirmation with the
 *        row count, since that's real lead data and deleting it is not
 *        recoverable.
 *      · absorbPaste_() now returns a full import audit and ▶ Build Stats
 *        reports it: export blocks detected, rows read, unique MLS#,
 *        duplicates collapsed, and the city mix found. Pasting the same
 *        export twice was always harmless (dedupe by MLS#, newest import
 *        wins) — but there was no way to notice a block that FAILED to
 *        parse, or that you forgot one. Now a missing block is visible
 *        immediately instead of turning up later as a wrong months-of-supply.
 *      · Published pages now carry the pipeline version + generation date
 *        in the footer (report, cma-rates, compare) — not just on the
 *        internal ▶ Run / Dashboard tabs.
 *      · west-jordan/ added to COMPARE_LOCATIONS and the LOCATIONS array
 *        inside buildCompareHtml_().
 *  v35 buildCompareData_() now also publishes s.segments (Resale SFH /
 *      Resale TH / New-Con SFH / New-Con TH — each with n, medPrice,
 *      medPpsf, medDom, stl). computeStats_() already builds this for the
 *      per-market report page; it just never made it into data.json.
 *      Lets any tool reading data.json (the compare page, or an external
 *      land/property dashboard) pull a real new-construction $/sqft
 *      instead of only the blended medPpsf3.
 ...
 * ------------------------------------------------
 * SETTING UP A NEW MARKET FROM A DUPLICATED SHEET:
 *   1. Duplicate an existing market's sheet
 *   2. Rename the spreadsheet to include the new city (Check Setup warns if not)
 *   3. Menu → 🆕 Set Up New Market → type the city name
 *   4. Menu → 🔑 Set GitHub Token (script properties don't reliably survive a copy)
 *   5. Menu → 📝 Setup Lead Form (creates a NEW form for this market)
 *   6. Paste ALL export blocks into 📥 Paste at once → ▶ Build Stats → 🌐 Publish
 */
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem('🆕 Set Up New Market (clears data)', 'setUpNewMarket')
...
const RUN_ACTIONS = [
  ...
  ['🆕 Set Up New Market', 'setUpNewMarket', 'DUPLICATED SHEET ONLY — clears data, log, cache, lead form + sets the new location']
];
...
function onRunEdit(e) {
  ...
  const fns = { buildStats, generateScript, publishToGitHub, checkSetup, publishCompareTool,
                generateCompareScript, setUpNewMarket };
...
function computeStats_() {
  ...
  return {
    ...
    reportMonth: Utilities.formatDate(now, Session.getScriptTimeZone(), 'MMMM yyyy'),
    reportMonthKey: Utilities.formatDate(now, Session.getScriptTimeZone(), 'yyyy-MM'),
...
function pricingModel_(solds) {
  ...
  // v38: cap, not re-derive — same rule as cmaModel_/cmaTHModel_, applied
  // here too since this stable 5-predictor model's fin/unf are the ones
  // actually PREFERRED and displayed everywhere (v34 fix: cmaRows_,
  // buildCompareData_ etc. all use pm.fin over cm.fin when pm.ok).
  const finRawPm = beta[2];
  const finCappedPm = Math.min(finRawPm, beta[1]);
  const finCapFiredPm = finRawPm > beta[1];
  return {
    ok: true, n: rows.length,
    intercept: beta[0], ag: beta[1], fin: finCappedPm, finRaw: finRawPm, finCapFired: finCapFiredPm,
    unf: beta[3], acre: beta[4], walk: beta[5],
...
function buildStats() {
  ...
  return abs ? importSummary_(abs) : 'rebuilt from existing data';
}

// v36 — one-line audit of what the paste actually contained, so a block that
// failed to parse (or one you forgot) is visible immediately.
function importSummary_(a) {
  const cities = Object.keys(a.cities).sort((x, y) => a.cities[y] - a.cities[x])
    .slice(0, 3).map(c => c + ' ' + a.cities[c]).join(', ');
  return a.blocks + ' block' + (a.blocks === 1 ? '' : 's') + ' · ' + a.rows + ' rows · ' +
    a.unique + ' unique MLS# · ' + a.dupes + ' dupes · ' +
    a.solds + ' sold / ' + a.actives + ' active · ' + cities;
}
...
// v37 — shared link-preview image for all published pages (report, cma-rates,
// compare). Points at the land engine's public asset rather than the
// Drive-hosted logoFileId, since uc?export=view is not reliably readable
// by link-preview crawlers (Drive requires a live session for that format).
const OG_IMAGE = 'https://lancea141-source.github.io/tm-market-update/assets/logo.svg';

// v36 — pipeline version + generation date, shown on every published page.
...
function buildHtml_(s) {
  ...
  <meta name="description" content="${CONFIG.farmName} real estate market update — live MLS data, updated ${s.updated}.">
  <meta property="og:type" content="website">
  <meta property="og:title" content="${CONFIG.farmName} Market Update">
  <meta property="og:description" content="Live MLS data · Updated ${s.updated} · ${CONFIG.agentBrand}">
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0}
    ...
    .report-month{font-size:3.2em;font-weight:bold;color:var(--cream);letter-spacing:.03em;line-height:1.05;margin-bottom:2px}
    ...
    .stamp{font-size:.9em;opacity:.75;margin-top:8px;display:block}
  ...
  <div class="index-card">
    <div class="report-month">${s.reportMonth.toUpperCase()}</div>
  ...
  <div class="pm-card">
    ...
    ${s.pm.finCapFired ? `<div class="kv-foot" style="color:var(--terra)">Capped at the above-grade rate — the raw model fit ($${Math.round(s.pm.finRaw)}/sqft) would have priced a finished basement above main-level space, which isn't a real signal in this sample.</div>` : ''}
  ...
  <a class="cta" style="background:var(--navy)" href="history/">📅 Past Reports</a>
  ...
  ${CONFIG.agentTagline} · Equal Housing Opportunity
  <span class="stamp">${buildStamp_()}</span></footer>
  </div></body></html>`;
  return html;
}
...
// 4. Menu → 🔑 Set GitHub Token (or Project Settings → Script Properties → GITHUB_TOKEN)
...
function publishToGitHub() {
  ...
  if (!token) throw new Error('Add GITHUB_TOKEN — menu → 🔑 Set GitHub Token.');
  ...
  // v38 — registry entry only. Self-backup-via-API (v37) was reverted:
  // script.googleapis.com reliably 403s under the default auto-created
  // Cloud project bound scripts get, and fixing it requires migrating to
  // a standard user-managed GCP project — out of proportion to what the
  // feature bought (one clickable link in one registry column). The
  // function is left below, unused, in case that migration is ever worth
  // doing. Script Link now always points at the script editor directly.
  try {
    registerToTracker_({
      project: 'Lance the Realtor',
      type: 'Apps Script Pipeline',
      pipeline: 'Market Update',
      area: CONFIG.farmName,
      version: VERSION,
      sourceUrl: SpreadsheetApp.getActiveSpreadsheet().getUrl(),
      publishedUrl: live,
      recordCount: s.sales12,
      status: 'Live',
      lastError: '',
      dataSource: 'WFRMLS/URE',
      scriptLink: 'https://script.google.com/home/projects/' + ScriptApp.getScriptId() + '/edit',
      changeNotes: 'Market update run',
      environment: 'Production'
    });
  } catch (e) {
    Logger.log('Tracker registration failed (publish itself succeeded): ' + e.message);
  }

  // v38 — one snapshot per calendar month (see saveMonthlySnapshot_ below).
  try { saveMonthlySnapshot_(g, token, s, live); }
  catch (e) { Logger.log('Monthly snapshot failed (publish itself succeeded): ' + e.message); }
...
// v35: added `segments`. v36: added `version`.
...
const COMPARE_LOCATIONS = [
  ...
  { path: 'west-jordan/', label: 'West Jordan' },
...
function generateCompareScript() {
  ...
  rows.push(['', '', '']);
  rows.push(['DATA AS OF', markets.map(m => m.label + ': ' + m.updated +
    (m.version ? ' (' + m.version + ')' : '')).join(' · '), '']);
...
function buildCompareHtml_() {
  ...
  <meta name="description" content="Cross-market comparison of every farm ${CONFIG.agentBrand} tracks.">
  <meta property="og:type" content="website">
  <meta property="og:title" content="Market Comparison — ${CONFIG.agentBrand}">
  <meta property="og:description" content="Live stats and CMA rates side by side across every market.">
  <meta property="og:image" content="${OG_IMAGE}">
  <meta name="twitter:card" content="summary_large_image">
  <style>
    :root{--terra:${b.terracotta};--olive:${b.olive};--navy:${b.navy};--camel:${b.camel};--cream:${b.cream}}
  ...
  ${CONFIG.agentPhone} · ${CONFIG.agentEmail} · Equal Housing Opportunity
  <span class="stamp">${buildStamp_()}</span></footer>
  </div>
  <script>
  ...
  const STAT_ROWS = [
    ['Updated', d => (d.updated || '—') + (d.version ? ' · ' + d.version : '')],
...
// ============================================================
// v38 — MONTHLY SNAPSHOT HISTORY
// ============================================================
// One saved copy per calendar month, keyed by reportMonthKey ('yyyy-MM').
// Publishing again within the same month overwrites that month's snapshot
// (so it always reflects the latest numbers for the month in progress);
// once the month rolls over, the prior month's folder is never touched
// again — a permanent, ever-growing archive, one entry per month.
function saveMonthlySnapshot_(g, token, s, liveUrl) {
  const base = g.path.replace(/index\.html$/, '');
  const histDir = base + 'history/' + s.reportMonthKey;
  ghPut_(g, token, histDir + '/index.html', buildHtml_(s),
    CONFIG.farmShort + ' snapshot ' + s.reportMonth + ' (' + VERSION + ')');
  ghPut_(g, token, histDir + '/data.json', JSON.stringify(buildCompareData_(s)),
    CONFIG.farmShort + ' snapshot data ' + s.reportMonth + ' (' + VERSION + ')');
  rebuildHistoryIndex_(g, token, base);
}

// Lists every saved month for this location by scanning the repo tree
// (same approach the land engine uses for its manifest scan), then writes
// a small branded index page linking to each one, newest first.
function listSavedMonths_(g, token, base) {
  const headers = { Authorization: 'Bearer ' + token, Accept: 'application/vnd.github+json' };
  const branchRes = UrlFetchApp.fetch(
    `https://api.github.com/repos/${g.owner}/${g.repo}/branches/${g.branch}`,
    { headers, muteHttpExceptions: true });
  if (branchRes.getResponseCode() !== 200) return [];
  const treeSha = JSON.parse(branchRes.getContentText()).commit.commit.tree.sha;
  const treeRes = UrlFetchApp.fetch(
    `https://api.github.com/repos/${g.owner}/${g.repo}/git/trees/${treeSha}?recursive=1`,
    { headers, muteHttpExceptions: true });
  if (treeRes.getResponseCode() !== 200) return [];
  const tree = JSON.parse(treeRes.getContentText()).tree || [];
  const prefix = base + 'history/';
  const pattern = new RegExp('^' + prefix.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '(\\d{4}-\\d{2})/index\\.html$');
  const months = [];
  tree.forEach(entry => {
    const m = entry.path.match(pattern);
    if (m) months.push(m[1]);
  });
  return months.sort().reverse(); // newest first
}

function rebuildHistoryIndex_(g, token, base) {
  const months = listSavedMonths_(g, token, base);
  const b = CONFIG.brand;
  const rows = months.map(mk => {
    const label = Utilities.formatDate(new Date(mk + '-01T00:00:00'), Session.getScriptTimeZone(), 'MMMM yyyy');
    return `<div class="card"><h4>${label}</h4><a href="${mk}/">View this month's report →</a></div>`;
  }).join('');
  const html = `<!DOCTYPE html><html><head>
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>${CONFIG.farmName} — Past Reports | ${CONFIG.agentName}</title>
  <meta property="og:image" content="${OG_IMAGE}">
  <style>
    :root{--terra:${b.terracotta};--olive:${b.olive};--navy:${b.navy};--camel:${b.camel};--cream:${b.cream}}
    *{box-sizing:border-box;margin:0}
    ...
    .updated{color:var(--olive);font-size:.85em;font-style:italic}
    .card{background:#fff;border-radius:10px;padding:14px 18px;margin:10px 0;border-top:4px solid var(--navy)}
    .card h4{margin:0 0 6px}
    .card a{color:var(--terra);font-weight:bold;text-decoration:none}
    footer{text-align:center;font-size:.72em;color:var(--olive);padding:18px 0}
  </style></head><body><div class="wrap">
  <header><h1>${CONFIG.farmName} — Past Reports</h1>
  <div class="updated">${months.length} month${months.length === 1 ? '' : 's'} archived</div></header>
  ${rows || '<div class="card">No archived months yet.</div>'}
  <footer><a href="../" style="color:var(--terra)">← Back to current report</a></footer>
  </div></body></html>`;
  ghPut_(g, token, base + 'history/index.html', html, CONFIG.farmShort + ' history index (' + VERSION + ')');
}
...
// v36: returns a full audit — blocks, rows, unique MLS#, dupes, city mix.
function absorbPaste_() {
  ...
  blocks.forEach(blk => {
    ...
    const hLow = blk.header.map(x => String(x).toLowerCase().trim());
    const si = hLow.indexOf('status');
    const mi = hLow.indexOf('mls#');
    const ci = hLow.indexOf('city');
    const rows = blk.rows.map(r => {
      ...
      if (mi > -1) {
        ...
        if (id) { if (seen[id]) dupes++; else seen[id] = 1; }
      }
      if (ci > -1) {
  ...
  return { solds: sc, actives: ac, blocks: blocks.length, rows: total,
           unique: Object.keys(seen).length, dupes: dupes, cities: cities };
}
...
// ============================================================
// v36 — 🆕 SET UP NEW MARKET (repurpose a DUPLICATED sheet)
// ============================================================
// A duplicated sheet carries five kinds of stale state, and missing any one
// of them produces a wrong-but-plausible report:
//   · Data rows        → old city's sales still feed the model
//   · Import Log       → history belongs to the old market
//   · cached stats     → a publish could serve the old market's numbers
//   · LEAD_FORM_URL    → new market's CTA points at the OLD city's form
//   · LOCATION         → wrong market until re-set
// GITHUB_TOKEN is deliberately KEPT (same account, same repo).
function setUpNewMarket() {
  const ui = SpreadsheetApp.getUi();
  const ss = SpreadsheetApp.getActive();

  const r = ui.prompt('🆕 Set Up New Market',
    'Use this ONLY on a duplicated sheet you are repurposing for a different city.\n\n' +
    'CLEARS: Data tab, Import Log, cached stats, lead-form link, stored location.\n' +
    'KEEPS:  GitHub token, all code.\n\n' +
    'Type the new market name to confirm (e.g. West Jordan):',
    ui.ButtonSet.OK_CANCEL);
  if (r.getSelectedButton() !== ui.Button.OK) return 'cancelled';
  const name = r.getResponseText().trim();
  if (!name) { ui.alert('Nothing typed — nothing cleared.'); return 'cancelled'; }

  const p = PropertiesService.getScriptProperties();
  const cleared = [];

  // 1. Data tab — wipe everything, restore just the standard header
  const dataSh = ss.getSheetByName('Data');
  if (dataSh) {
    const rows = Math.max(0, dataSh.getLastRow() - 1);
    dataSh.clear();
    cleared.push(rows + ' data rows');
  }

  // 2. Import Log
  const logSh = ss.getSheetByName('Import Log');
  if (logSh) { logSh.clear(); cleared.push('import log'); }

  // 3. Cached stats (6-hour cache would otherwise serve the old market)
  try { CacheService.getScriptCache().remove('stats'); cleared.push('cached stats'); } catch (e) {}

  // 4. Lead form link — the dangerous one
  if (p.getProperty('LEAD_FORM_URL')) {
    p.deleteProperty('LEAD_FORM_URL');
    cleared.push('lead-form link');
  }
  ScriptApp.getProjectTriggers().forEach(t => {
    if (t.getHandlerFunction() === 'onLeadSubmit') ScriptApp.deleteTrigger(t);
  });

  // 5. Stored location — replaced below
  p.deleteProperty('LOCATION');

  // 6. Form Responses — SEPARATE confirmation. This is real lead data from
  //    the old market and deleting it is not recoverable.
  ss.getSheets().forEach(sh => {
    const n = sh.getName();
    if (n.indexOf('Form Responses') !== 0) return;
    const n2 = Math.max(0, sh.getLastRow() - 1);
    const ok = ui.alert('Delete "' + n + '"?',
      'This tab holds ' + n2 + ' lead response' + (n2 === 1 ? '' : 's') + ' from the OLD market.\n\n' +
      'Deleting is permanent. Choose NO to keep it.',
      ui.ButtonSet.YES_NO);
    if (ok === ui.Button.YES) { ss.deleteSheet(sh); cleared.push(n + ' (' + n2 + ' responses)'); }
  });

  // 7. Set the new location in the same step, so it can't be forgotten
  const L = resolveLocation_(name, ui);
  if (!L) { ui.alert('Cleared, but no location set — run 📍 Set Location before publishing.'); return 'cleared, no location'; }
  p.setProperty('LOCATION', JSON.stringify(L));
  applyLocation_();
  buildRunTab_();

  ui.alert('🆕 ' + L.farmName + ' is ready.\n\n' +
    'Cleared: ' + (cleared.length ? cleared.join(', ') : 'nothing to clear') + '\n' +
    'Publishes to: https://' + CONFIG.github.owner + '.github.io/' + CONFIG.github.repo + '/' +
    L.path.replace(/index\.html$/, '') + '\n\n' +
    'NEXT:\n' +
    '1. Rename this spreadsheet to include "' + L.farmName + '"\n' +
    '2. 🔑 Set GitHub Token (properties don\'t reliably survive a copy)\n' +
    '3. 📝 Setup Lead Form (creates a NEW form for this market)\n' +
    '4. Paste ALL export blocks into 📥 Paste at once → ▶ Build Stats\n' +
    '5. 🌐 Publish to GitHub');
  return L.farmName + ' ready';
}
...
function cmaModel_(solds) {
  ...
  // v38: cap, not re-derive. A finished basement should not price above
  // main-floor living space — that's a symptom of a free-fit hedonic model
  // under-constrained by thin basement-variation data, not a real market
  // signal. Only intervenes when the fit actually violates this; markets
  // where fin already sits below l1 are unaffected. finRaw is kept for
  // anyone who wants to see what the uncapped fit produced.
  const finRaw = beta[3];
  const finCapped = Math.min(finRaw, beta[1]);
  const finCapFired = finRaw > beta[1];
  return {
    ok: true, n: rows.length, r2: f.r2, medErr: f.medErr,
    base: beta[0], l1: beta[1], up: beta[2], fin: finCapped, finRaw: finRaw, finCapFired: finCapFired,
    unf: beta[4], acre: beta[5], walk: beta[6], gar: beta[7], age: beta[8],
...
function cmaRows_(cm, pm) {
  ...
  const usePm = pm && pm.ok;
  const fin = usePm ? pm.fin : cm.fin;
  const unf = usePm ? pm.unf : cm.unf;
  const finCapFired = usePm ? pm.finCapFired : cm.finCapFired;
  const finRaw = usePm ? pm.finRaw : cm.finRaw;
  const finNote = finCapFired
    ? '🟠 CAPPED at the above-grade rate — the model\'s free fit produced $' + Math.round(finRaw) +
      '/sqft, which would price a finished basement above main-level living space. Capped to the above-grade rate; not a real market signal in this sample.'
    : '🟢 from the stable 5-component model';
  ...
  return [
    ...
    ['Finished basement sqft', '$' + Math.round(fin) + '/sqft', finNote],
...
function buildCmaHtml_(s) {
  ...
  <meta name="description" content="What each feature is actually worth in ${CONFIG.farmName} — CMA adjustment rates fit on ${cm.n} resale sales.">
  <meta property="og:type" content="website">
  <meta property="og:title" content="The ${CONFIG.farmShort} Adjustment Table">
  <meta property="og:description" content="Field-by-field CMA rates for ${CONFIG.farmName}, refit on every publish.">
  <meta property="og:image" content="${OG_IMAGE}">
  <meta name="twitter:card" content="summary_large_image">
  <style>
    :root{--terra:${b.terracotta};--olive:${b.olive};--navy:${b.navy};--camel:${b.camel};--cream:${b.cream}}
...
function cmaTHModel_(solds) {
  ...
  const bathSizeCorr = corr_(bathVals, sizeVals);
  const bathGarCorr = corr_(bathVals, garVals);
  // v38: same cap as cmaModel_ — finished basement should not exceed the
  // above-grade rate (b[1] here, since townhomes have no L1/upper split).
  const finRawTH = b[2];
  const finCappedTH = Math.min(finRawTH, b[1]);
  const finCapFiredTH = finRawTH > b[1];
  return { ok: true, n: rows.length, r2: f.r2, medErr: f.medErr,
    base: b[0], ag: b[1], fin: finCappedTH, finRaw: finRawTH, finCapFired: finCapFiredTH,
    unf: b[3], gar: b[4], age: b[5],
    bathRate: bathRate, fb: bathRate, tq: bathRate * 0.75, hb: bathRate * 0.5,
...
function cmaRowsTH_(m) {
  ...
  const finNote = m.finCapFired
    ? '🟠 CAPPED at the above-grade rate — free fit produced $' + Math.round(m.finRaw) +
      '/sqft, which would exceed above-grade value; capped, not a real signal here'
    : '🟡 many TH have no basement — thin signal';
  return [
    ['Above-grade sqft', '$' + Math.round(m.ag) + '/sqft', '🟢'],
    ['Finished basement sqft', '$' + Math.round(m.fin) + '/sqft', finNote],
...
const LOC_PRESETS = {
  ...
  '5': { farmName: 'West Jordan', farmShort: 'West Jordan',
    areaLine: 'all of West Jordan',
    scopeNote: 'West Jordan city limits only',
    numbersLine: "Here's West Jordan in numbers.",
    agentTagline: 'Data-driven real estate in West Jordan',
    path: 'west-jordan/index.html' }
};
...
// v36 — shared by 📍 Set Location and 🆕 Set Up New Market so a market can
// be resolved from a typed name without prompting twice.
function resolveLocation_(t, ui) {
  if (!t) return null;
  let L = LOC_PRESETS[t];
  if (!L) {
  ...
  if (!L) {
    ...
    if (ui) {
      const s = ui.prompt('📍 Scope for ' + name,
        ...
        ui.ButtonSet.OK_CANCEL);
      if (s.getSelectedButton() === ui.Button.OK && s.getResponseText().trim()) {
        scope = s.getResponseText().trim();
      }
    }
...
function setLocation() {
  ...
  const L = resolveLocation_(t, ui);
  if (!L) { ui.alert('Could not resolve that market name.'); return; }
...
// ============================================================
// v37 — PROJECT REGISTRY TRACKER + SELF-BACKUP
// ============================================================
/**
 * REGISTER RUN SNIPPET — v1.0
 * Self-reports pipeline runs to the Project Registry tracker sheet.
 */
const TRACKER_SHEET_ID = '1gtsQwS67i2spgtwgH-MeiFO_qqJJZtkSkw18BZHZ3Sg';
const TRACKER_COLUMNS = [
  'Timestamp', 'Project', 'Type', 'Pipeline', 'Area', 'Version',
  'Source URL', 'Published URL', 'Record Count', 'Status', 'Last Error',
  'Data Source / Language', 'Script / Project Link', 'Change Notes', 'Environment'
];
const TRACKER_KEY_COLS = ['Project', 'Pipeline', 'Area'];

function registerToTracker_(data) {
  try {
    ...
    const now = new Date();
    const row = TRACKER_COLUMNS.map(col => {
      switch (col) {
        ...
        case 'Source URL': return data.sourceUrl || '';
        case 'Published URL': return data.publishedUrl || '';
        case 'Record Count': return data.recordCount ?? '';
        case 'Status': return data.status || 'Live';
        case 'Last Error': return data.lastError || '';
        case 'Data Source / Language': return data.dataSource || '';
        case 'Script / Project Link': return data.scriptLink || '';
        case 'Change Notes': return data.changeNotes || '';
        case 'Environment': return data.environment || 'Production';
        default: return '';
      }
    });
    ...
    const headerIdx = {};
    TRACKER_COLUMNS.forEach((c, i) => headerIdx[c] = i);

    let matchRow = -1;
    for (let i = 1; i < dashData.length; i++) {
      const r = dashData[i];
      const isMatch = TRACKER_KEY_COLS.every(k => (r[headerIdx[k]] || '') === (row[headerIdx[k]] || ''));
      if (isMatch) { matchRow = i + 1; break; }
    }

    if (matchRow === -1) {
      dash.appendRow(row);
    } else {
      const existingVersion = dashData[matchRow - 1][headerIdx['Version']];
      if (existingVersion && existingVersion !== row[headerIdx['Version']]) {
        archive.appendRow(dashData[matchRow - 1]);
      }
      dash.getRange(matchRow, 1, 1, row.length).setValues([row]);
    }
  } catch (e) {
    Logger.log('registerToTracker_ failed: ' + e.message);
  }
}

// v37 — pushes this script's OWN current source to GitHub, once per VERSION,
// via the Apps Script API (script.googleapis.com). Requires:
//   1. Google Apps Script API enabled at script.google.com/home/usersettings
//   2. OAuth scope 'https://www.googleapis.com/auth/script.projects.readonly'
//      added to this project's manifest (View > Show manifest file >
//      appsscript.json > add to "oauthScopes"). First publish after adding
//      the scope triggers a normal one-time consent prompt.
// Returns the raw GitHub URL for the backup (existing one if already backed
// up this version, freshly created one if not, or '' if the API isn't
// enabled/authorized yet — publishing still proceeds either way).
function backupSourceToGithub_(token) {
  const p = PropertiesService.getScriptProperties();
  const versionKey = 'BACKED_UP_VERSION';
  const urlKey = 'BACKUP_URL';
  if (p.getProperty(versionKey) === VERSION) {
  ...
  try {
    ...
    if (res.getResponseCode() !== 200) {
      Logger.log('Self-backup: could not read own source (' + res.getResponseCode() +
        '). Enable the Apps Script API at script.google.com/home/usersettings and re-publish.');
      return p.getProperty(urlKey) || '';
    }
    const proj = JSON.parse(res.getContentText());
    const gsFiles = (proj.files || []).filter(f => f.type === 'SERVER_JS');
    if (!gsFiles.length) return p.getProperty(urlKey) || '';
    // The main pipeline file is the largest SERVER_JS file in the project.
    gsFiles.sort((a, b) => (b.source || '').length - (a.source || '').length);
    const source = gsFiles[0].source;

    const g = CONFIG.github;
    const slugPart = (g.path.replace(/index\.html$/, '').replace(/\/$/, '')) || 'traverse-mountain';
    const dateStr = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
    const safeFarm = CONFIG.farmShort.replace(/\s+/g, '-');
    const backupPath = 'backups/property/' + slugPart + '/' + safeFarm + '_' + VERSION + '_' + dateStr + '.gs';

    ghPut_(g, token, backupPath, source, 'Backup ' + CONFIG.farmName + ' ' + VERSION + ' source');

    const rawUrl = 'https://raw.githubusercontent.com/' + g.owner + '/' + g.repo + '/' + g.branch + '/' + backupPath;
    p.setProperty(versionKey, VERSION);
    p.setProperty(urlKey, rawUrl);
    return rawUrl;
  } catch (e) {
    Logger.log('Self-backup failed: ' + e.message);
    return p.getProperty(urlKey) || '';
  }
}
